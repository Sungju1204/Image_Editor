...
const handleCanvasMouseMove = (event) => {
  ...
  if (!foundHover) {
    ...
    if (canvasRef.value) {
      canvasRef.value.style.cursor = isDrawingLine.value ? 'crosshair' : 'default'
    }
  }
...
const handleDuplicateLine = (lineId) => {
  ...
  const length = Math.sqrt(dx * dx + dy * dy)
  const offset = line.type === 'horizontal' ? { x: 0, y: length } : { x: length, y: 0 }
...
const redrawCanvas = () => {
  ...
  props.lines.forEach((line) => {
    const isHovered = hoveredLineId.value === line.id
    const isDragging = draggingLine.value === line.id
    
    let strokeColor = line.type === 'horizontal' ? COLORS.HORIZONTAL_LINE : COLORS.VERTICAL_LINE

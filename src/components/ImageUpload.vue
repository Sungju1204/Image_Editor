...
const handleDirSelect = (event) => {
  const files = Array.from(event.target.files)
  // 폴더 선택 시 하위 모든 파일이 files로 들어옵니다(Chrome 기반)
...
<template>
  <div class="app-container">
    ...
    >
      ...
      <p class="upload-hint">여러 이미지 또는 폴더를 선택할 수 있습니다</p>
      <div class="btn-row">
        <button class="choose-file-btn" @click="triggerFileInput">파일 선택</button>
        <button class="choose-file-btn secondary" @click="triggerDirInput">폴더 선택</button>
      </div>
...
.choose-file-btn.secondary {
  background-color: transparent;
  color: #888;
  border: 2px solid #444;
}

.choose-file-btn.secondary:hover {
  border-color: #888;
  color: #fff;
  background-color: #2a2a2a;
}
